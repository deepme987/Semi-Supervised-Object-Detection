...
parser.add_argument("--high_lr", default=5e-3, type=float,
                    help="lr for rcnn")
parser.add_argument("--low_lr", default=1e-4, type=float,
                    help="lr for transfer layer in backbone")
parser.add_argument("--super_low_lr", default=1e-6, type=float,
...
def main():
    ...
    for name, param in model.named_parameters():
        if "backbone.body" in name:
            if "layer1" in name or "backbone.body.conv1" in name:
